# OpenHDMap Threat model generated by ChatGPT

_This README file contains a threat model for the OpenHDMap application as generated by ChatGPT by feeding it the original README file (found at https://github.com/wheelos/OpenHDMap)_

## Overview of key terms and definitions
_Question to ChatGPT_:
```
Can you please speed me up with key terms and definition to understand the following github readme 
Navigation Menu
Sign in
wheelheart
/
...
License
 Apache-2.0 license
 221 stars  129 forks  Branches  Tags  Activity
Code
Issues
2
Pull requests
wheelheart/OpenHDMap
Folders and files
Name	
Latest commit
daohu527
...
```
_I merged the output of ChatGPT and the original README file in the following:_
...
_I further interacted with chatgpt to let the application deploy on a K8s-based MEC_:

```
How would this system be deployed on MEC. Assume each MEC host runs on K8s cluster that accesible for cars using 5G and UPF. Assume multiple MEC hosts exist for different geo regions and thr HDMap instances must collaborate to exchange part of their point clouds
```
_Output of ChatGPT:_
...
## Pytm Threat model Generated by ChatGPT

_Question to ChatGPT_

```
Can you generate me a pytm dfd and  stride threats for the above deployment. Also first introduce the basis of pytm
```
_Response of ChatGPT (Note this ChatGPT generated pytm script does not compile!):_
...
# _The above overview was really limited so I asked ChatGPT for a detailed overview_

## Detailed threat repository
```
Can  you be as exhaustive as possible for the Spoofing threats
```
...
```
Now do the same for Tampering
```
_ChatGPTs response:_
...
_Now I asked ChatGPT to zoom into the specifics of K8s and apply threat modeling to it:_

```
Each K8s node has specifc components and data stores. E.g. master node has the control plane, thereafter on every node there is the kubelet, there are CRI, CSI, and CNI plugins. Can you present these and other components more exhaustively in a pytm dfd that starts from the pytm did for hdmap
```
_Response of ChatGPT:_
...
_Now I extended to zoom into the threat models in more detail_

```
Now append a detailed list of STRIDE threats for K8s and its interaction with HDMap processes
```

_ChatGPT's response_:
...
Now I asked ChatGPT to do risk analysis

```
For all threats generated in this chatgpt conversation, please list the severity and likelyhood of each threat
```
